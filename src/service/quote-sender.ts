...
export class QuoteSender {
    ...
    private checkCrossedQuotes = (side: Models.Side, px: number): boolean => {
        ...
        var doesQuoteCross = oppSide === Models.Side.Bid ? (q, px) => q.price >= px : (q, px) => q.price <= px;
    ...
    private sendQuote = (timeOfQuote: Date): void => {
        var quote = this._quotingEngine.latestQuote;

        var askStatus = Models.QuoteStatus.Held;
        var bidStatus = Models.QuoteStatus.Held;
        //TODO: Self Trade Allowed-->NO Trade Yourself OR Self Trade Not Allowed-->No Trade Yourself
        ...
        if (askStatus === Models.QuoteStatus.Live) {
            askAction = this._quoter.updateQuote(new Models.Timestamped(quote.ask, timeOfQuote), Models.Side.Ask);
        }
        else {
            askAction = this._quoter.cancelQuote(new Models.Timestamped(Models.Side.Ask, timeOfQuote));
        }

        var bidAction: Models.QuoteSent;
        if (bidStatus === Models.QuoteStatus.Live) {
            bidAction = this._quoter.updateQuote(new Models.Timestamped(quote.bid, timeOfQuote), Models.Side.Bid);
        }
        else {
            bidAction = this._quoter.cancelQuote(new Models.Timestamped(Models.Side.Bid, timeOfQuote));
        }
